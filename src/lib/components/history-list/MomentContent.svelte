...
<ol style={`--graph-width: ${graph.width}px`}>
...
<style>
    ...
    li {
        font-size: 0.75rem;
        display: grid;
        grid-template-columns: 4rem var(--graph-width) 16px minmax(0, 1fr) auto;
        align-items: center;
        gap: 0.5rem;
        &:not(:last-child) {
            box-shadow: inset 0 -1px var(--el-border-color-default);
        }
    }
    .visit-details {
        min-width: 0;
