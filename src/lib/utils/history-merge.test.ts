...
const visit = (
  ...
  url: string,
  visitTime?: number,
): HistoryVisit => ({ visitId, url, visitTime });
...
describe("mergeHistoryVisits", () => {
  it("replaces accepted URLs and merges interleaved visits without accepting stale updates", () => {
    ...
    expect(result.map((item) => item.visitId)).toEqual([
      "a-new",
      "b",
      "d",
      "c",
    ...
    expect(updates.map((item) => item.visitId)).toEqual([
      "stale",
      "a-new",
      "d",
      "a-old",
  ...
  it("removes accepted URLs when Chrome returns no visits and handles empty inputs", () => {
    ...
    expect(mergeHistoryVisits(history, [], new Set(["a"]))).toEqual([
      history[1],
    ]);
    expect(mergeHistoryVisits([], history, new Set(["a", "b"]))).toEqual(
      history,
    );
  ...
  it("preserves stable tie order and the loader's missing-timestamp fallback", () => {
    ...
    expect(
      mergeHistoryVisits(history, updates, new Set(["c"])).map(
        (item) => item.visitId,
      ),
